CREATE DEFINER=`root`@`localhost` FUNCTION `ordersetlist`(srno int) RETURNS varchar(4000) CHARSET latin1
BEGIN
   /*
  ...
  */
  ...
  DECLARE me CURSOR for
          select CONCAT(CAST(order_set_no as char), ' - ', korder_set.title_nm, '<BR>')
          from tip_tracker
